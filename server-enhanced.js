...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    cluster: {
      ...
      context: clusterConnected ? kc.getCurrentContext() : null,
      server: clusterConnected ? kc.getCurrentCluster()?.server : null
    }
  });
});
...
server.listen(PORT, () => {
  ...
  logger.info(`Context: ${clusterConnected ? kc.getCurrentContext() : 'N/A'}`);
  logger.info(`API Server: ${clusterConnected ? kc.getCurrentCluster()?.server : 'N/A'}`);
